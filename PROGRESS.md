...
- [x] **S-01** Docker Desktop 설치 확인 및 버전 체크 (≥ v4.0)
  - 완료일: 2026-03-03
  > ✅ 확인: Docker Engine 버전 `28.3.2` 확인 완료.
...
- [x] **S-04** Frontend 의존성 설치 검증 (`npm install`)
  - 완료일: 2026-02-27
- [x] **S-05** `docker-compose up` 으로 전체 스택 기동 확인
  - 완료일: 2026-03-03
  > ✅ 확인: `FRONTEND_PORT=43000 docker-compose up -d --build` 기준 backend/frontend/mlflow 기동 검증 완료.
...
- [x] **P1-08** 데이터 미리보기 API (`GET /api/data/{file_id}/preview`)
  - 완료일: 2026-02-27
- [x] **P1-09** Upload 페이지 UI 컴포넌트 (`pages/UploadPage.jsx`)
  - 완료일: 2026-02-28
  - 요구사항: 드래그앤드롭, 파일 형식 안내, 샘플 데이터 다운로드 버튼
- [x] **P1-10** DataPreview 컴포넌트 (상위 100행 테이블, 결측값 하이라이트)
  - 완료일: 2026-02-28

### [W5-W6] AutoML 학습 엔진

- [x] **P1-11** FLAML AutoML 래퍼 서비스 (`services/automl_service.py`)
  - 완료일: 2026-02-28
  - 요구사항: 분류/회귀 자동 판별, time_budget=120초 기본값
- [x] **P1-12** 학습 API (`POST /api/train/start`) + BackgroundTasks 비동기 처리
  - 완료일: 2026-02-28
- [x] **P1-13** SSE 진행률 스트리밍 (`GET /api/train/progress/{session_id}`)
  - 완료일: 2026-02-28
- [x] **P1-14** SHAP 피처 중요도 계산 및 결과 저장
  - 완료일: 2026-02-28
- [x] **P1-15** 학습 설정 UI (`pages/SetupPage.jsx`) — 타겟/피처 선택 위저드
  - 완료일: 2026-02-28
- [x] **P1-16** 학습 진행 UI (`pages/TrainingPage.jsx`) — 원형 프로그레스 + 로그
  - 완료일: 2026-02-28

### [W7] 결과 시각화 & 예측

- [x] **P1-17** 결과 대시보드 API (`GET /api/train/results/{model_id}`)
  - 완료일: 2026-02-28
- [x] **P1-18** 결과 대시보드 UI (`pages/ResultsPage.jsx`)
  - 완료일: 2026-02-28
  - 요구사항: 정확도 카드, 혼동행렬, SHAP 바 차트 (recharts)
- [x] **P1-19** 단건 예측 API (`POST /api/predict/single`)
  - 완료일: 2026-02-28
- [x] **P1-20** 배치 예측 API (`POST /api/predict/batch`) — CSV 업로드 방식
  - 완료일: 2026-02-28
- [x] **P1-21** 예측 입력 UI (`pages/PredictPage.jsx`)
  - 완료일: 2026-02-28

### [W8] 리포트 & 패키징

- [x] **P1-22** PDF 리포트 Jinja2 템플릿 작성 (`templates/report.html`)
  - 완료일: 2026-02-28
- [x] **P1-23** PDF 생성 서비스 (`services/report_service.py`) + WeasyPrint
  - 완료일: 2026-02-28
- [x] **P1-24** PDF 다운로드 API (`GET /api/report/{model_id}`)
  - 완료일: 2026-02-28
...
- [x] **P2-01** MLflow 도커 서비스 추가 (`docker-compose.yml` 확장)
  - 완료일: 2026-02-28
- [x] **P2-02** `automl_service.py` 에 MLflow 자동 기록 연동
  - 완료일: 2026-02-28
- [x] **P2-03** 실험 목록 조회 API (`GET /api/experiments`)
  - 완료일: 2026-02-28
- [x] **P2-04** 실험 상세 조회 API (`GET /api/experiments/{run_id}`)
  - 완료일: 2026-02-28

### [W3-W4] 모델 레지스트리

- [x] **P2-05** 모델 등록 API (`POST /api/registry/register`)
  - 완료일: 2026-02-28
- [x] **P2-06** 운영 모델 전환 API (`PUT /api/registry/{model_name}/stage`)
  - 완료일: 2026-02-28
- [x] **P2-07** 모델 히스토리 페이지 UI (`pages/ModelHistoryPage.jsx`)
  - 완료일: 2026-02-28
- [x] **P2-08** 모델 레지스트리 UI (`pages/RegistryPage.jsx`)
  - 완료일: 2026-02-28

### [W5-W6] 드리프트 감지

- [x] **P2-09** Evidently 드리프트 감지 서비스 (`services/drift_service.py`)
  - 완료일: 2026-02-28
- [x] **P2-10** APScheduler 주간 자동 실행 설정 (`scheduler.py`)
  - 완료일: 2026-02-28
- [x] **P2-11** 드리프트 알림 저장 API 및 DB 모델
  - 완료일: 2026-02-28
- [x] **P2-12** 드리프트 대시보드 UI (`pages/DriftPage.jsx`)
  - 완료일: 2026-02-28
  - 요구사항: 게이지 차트, 알림 타임라인, 재학습 버튼

### [W7] 비교 분석

- [x] **P2-13** 실험 비교 API (`POST /api/experiments/compare`)
  - 완료일: 2026-02-28
- [x] **P2-14** 실험 비교 UI — 레이더 차트 (recharts)
  - 완료일: 2026-02-28
- [x] **P2-15** A/B 예측 비교 API (`POST /api/predict/ab-compare`)
  - 완료일: 2026-02-28
- [x] **P2-16** A/B 비교 UI — 나란히 결과 표시
  - 완료일: 2026-02-28

### [W8] 재학습 자동화

- [x] **P2-17** 원클릭 재학습 API (`POST /api/train/retrain/{model_id}`)
  - 완료일: 2026-02-28
- [x] **P2-18** 재학습 트리거 조건 설정 UI (임계값 슬라이더)
  - 완료일: 2026-02-28
- [x] **P2-19** `requirements.txt` Phase 2 의존성 추가 (mlflow, evidently, apscheduler)
  - 완료일: 2026-02-28
- [x] **P2-20** Phase 2 전체 통합 테스트
  - 완료일: 2026-02-28
- [x] **P2-21** 사이드바 네비게이션 업데이트 (Phase 2 메뉴 추가)
  - 완료일: 2026-02-28
- [x] **P2-22** Phase 2 `docker-compose.yml` 최종 업데이트
  - 완료일: 2026-02-28
