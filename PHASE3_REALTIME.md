...
- [ ] `/tmp/mas-watch` 에 CSV 파일 복사 → 실시간 대시보드에 자동 반영 확인
- [ ] 불량 확률 70% 이상 데이터 포함 CSV → 카카오 알림 수신 확인
- [ ] 매일 오전 8시 이메일 리포트 수신 확인 (또는 수동 테스트)
- [ ] admin / operator / viewer 계정 각각 접근 권한 테스트
- [ ] 스마트폰(모바일) 브라우저에서 대시보드 정상 표시 확인
- [ ] Docker 재시작 후 파일 감시 자동 재개 확인
